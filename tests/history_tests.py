"""Cursor pagination, durable backup, retention and retry uniqueness."""
import asyncio
...
class HistoryTests(unittest.IsolatedAsyncioTestCase):
    async def test_history_pagination_retention_backup(self):
        with tempfile.TemporaryDirectory() as directory:
            path=Path(directory)/'community.sqlite3'
            h=History(path,retention_days=1,max_rows=205)
            await h.open()
            try:
                for i in range(211):
                ...
                await h.append({'kind':'log','time':int(time.time())-172800,'name':'Test','text':'Expired',
                    'frequency':7100000,'mode':'LSB','call':'Test','client_key':'b'*32,'request_id':'c'*32})
                self.assertEqual((await h.history('log'))['events'],[])
                # Backup while the writer connection is still open in WAL mode.
                ...
                with sqlite3.connect(output.decode().strip()) as copied:
                    ...
                    self.assertEqual(copied.execute('SELECT COUNT(*) FROM events').fetchone()[0],205)
            finally:
                await h.close()
